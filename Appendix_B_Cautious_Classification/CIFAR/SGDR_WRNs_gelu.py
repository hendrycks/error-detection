...
from lasagne.layers import InputLayer, DenseLayer, DropoutLayer, batch_norm
try:
...
def load_data(dataset):
    ...
    if dataset == 'CIFAR-10':
        for j in range(5):
          d = unpickle('/home-nfs/dan/cifar_data/cifar-10-batches-py/data_batch_'+str(j+1))
          x = d['data']
          y = d['labels']
          xs.append(x)
          ys.append(y)

        d = unpickle('/home-nfs/dan/cifar_data/cifar-10-batches-py/test_batch')
        xs.append(d['data'])
        ys.append(d['labels'])
    if dataset == 'CIFAR-100':
        d = unpickle('/home-nfs/dan/cifar_data/cifar-100-python/train')
        x = d['data']
        y = d['fine_labels']
        xs.append(x)
        ys.append(y)

        d = unpickle('/home-nfs/dan/cifar_data/cifar-100-python/test')
        xs.append(d['data'])
...
def ResNet_FullPre_Wide(input_var=None, nout=10,  n=3, k=2, dropoutrate = 0):
    ...
    network = DenseLayer(avg_pool, num_units=nout, W=HeNormal(), nonlinearity=lasagne.nonlinearities.identity)

    conf_layer_1 = DenseLayer(
        ElemwiseSumLayer([DenseLayer(bn_post_relu, num_units=256, nonlinearity=lasagne.nonlinearities.identity),
                          DenseLayer(network, num_units=256, nonlinearity=lasagne.nonlinearities.identity)]),
        num_units=128, nonlinearity=f)

    conf = DenseLayer(conf_layer_1, num_units=1, nonlinearity=lasagne.nonlinearities.sigmoid)
    network = NonlinearityLayer(network, nonlinearity=lasagne.nonlinearities.softmax)
...
def main(mode="softmax", dataset = 'CIFAR-10', iscenario = 0, n=5, k = 1, num_epochs=82, model = None, irun = 0, Te = 2.0, E1 = 41, E2 = 61, E3 = 81,
         lr=0.1, lr_fac=0.1, reg_fac=0.0005, t0=math.pi/2.0, Estart = 0, dropoutrate = 0, multFactor = 1):

    # modes: "softmax" and "sigmoid"

    # Check if CIFAR data exists
    if dataset == 'CIFAR-10':
        if not os.path.exists("/home-nfs/dan/cifar_data/cifar-10-batches-py/"):
            print("CIFAR-10 dataset can not be found. Please download the dataset from 'https://www.cs.toronto.edu/~kriz/cifar.html'.")
            return
        nout = 10
    if dataset == 'CIFAR-100':
        if not os.path.exists("/home-nfs/dan/cifar_data/cifar-100-python/"):
    ...
    network, sigmoid_output = ResNet_FullPre_Wide(input_var, nout,  n, k, dropoutrate)
    ...
    if mode == "softmax":
        ...
        conf = lasagne.layers.get_output(sigmoid_output)
        conf_labels = T.cast(T.eq(T.argmax(prediction, axis=1), target_var), "int32")
        loss = lasagne.objectives.categorical_crossentropy(prediction, target_var).mean() +\
               lasagne.objectives.binary_crossentropy(conf, conf_labels).mean()
    ...
    if mode == "softmax":
        conf_test = T.max(test_prediction, axis=1)   # using softmax prediction probability as confidence
    elif mode == "sigmoid":
        conf_test = lasagne.layers.get_output(sigmoid_output, deterministic=True)

    test_calibration = T.mean((2*conf_test - 1) *
                              (2 * T.cast(T.eq(T.argmax(test_prediction, axis=1), target_var), theano.config.floatX) - 1))
    test_model_score = T.mean(T.cast(T.eq(T.argmax(test_prediction, axis=1), target_var), theano.config.floatX) * ((2*conf_test - 1) *
                              (2 * T.cast(T.eq(T.argmax(test_prediction, axis=1), target_var), theano.config.floatX) - 1) + 1)/2)

    # Compile a second function computing the validation loss and accuracy:
    val_fn = theano.function([input_var, target_var], [test_loss, test_acc, test_calibration, test_model_score])
    ...
    if model is None:
        ...
        for epoch in range(num_epochs):
            ...
            if epoch+1 == TeNext:     # time to restart TODO: remove second condition
                tt = 0                  # by setting to 0 we set lr to lr_max, see above
                Te = int(Te * multFactor)   # change the period of restarts
            ...
            for batch in iterate_minibatches(X_test, Y_test, 500, shuffle=False):
                inputs, targets = batch
                err, acc, align, score = val_fn(inputs, targets)
                val_err += err
                val_acc += acc
                val_align += align
                val_model_score += score
                val_batches += 1
            ...
            print("  validation calibration:\t\t{:.2f} %".format(val_align / val_batches * 100))
            print("  validation model score:\t\t{:.2f} %".format(val_model_score/ val_batches * 100))
            ...
            if epoch % 10 == 0 or epoch > 48:
                filesave = "./data/network_{}_{}_{}.npz".format(iscenario,irun,epoch)
                np.savez(filesave, *lasagne.layers.get_all_param_values(network))
                # pickle.dump(lasagne.layers.get_all_param_values(network),
                #             open("./data/network_{}_{}_{}.pkl".format(iscenario,irun,epoch), 'wb'))
                print('Saved')
        ...
        # final save
        filesave = "./data/network_{}_{}_end.npz".format(iscenario,irun)
        np.savez(filesave, *lasagne.layers.get_all_param_values(network))
        # pickle.dump(lasagne.layers.get_all_param_values(network),
        #             open("./data/network_{}_{}_end.pkl".format(iscenario,irun), 'wb'))
    ...
    for batch in iterate_minibatches(X_test, Y_test, 500, shuffle=False):
        ...
        val_model_score += score
        val_batches += 1
    print("Test loss:\t\t{:.6f}".format(val_err / val_batches))
    print("Test accuracy:\t\t{:.2f} %".format(val_acc / val_batches * 100))
    print("Test calibration:\t\t{:.2f} %".format(val_align / val_batches * 100))
    print("Test model score:\t\t{:.2f} %".format(val_model_score / val_batches * 100))
...
if __name__ == '__main__':
    ...
    t0 = math.pi/2.0
    Te = 50
    dropoutrate = 0.0  # TODO: note this is on
    multFactor = 1
    num_epochs = 50
    ...
    if (iscenario == 3):    dataset = 'CIFAR-10';     n = 6;  k = 2;  Te = 50;      mode = "softmax"      # 40-4
    if (iscenario == 4):    dataset = 'CIFAR-10';     n = 6;  k = 2;  Te = 50;      mode = "sigmoid"      # 40-4
    if (iscenario == 5):    dataset = 'CIFAR-100';    n = 6;  k = 2;  Te = 50;      mode = "softmax"      # 40-4
    if (iscenario == 6):    dataset = 'CIFAR-100';    n = 6;  k = 2;  Te = 50;      mode = "sigmoid"      # 40-4
    ...
    if (iscenario == 13):   dataset = 'CIFAR-100';    n = 6;  k = 4;  Te = 50;      # 40-4
    if (iscenario == 14):   dataset = 'CIFAR-100';    n = 4;  k = 10;  Te = 100;
    if (iscenario == 15):   dataset = 'CIFAR-100';    n = 6;  k = 2;  Te = 1;      multFactor = 2;
    ...
    irun = 1
    main(mode, dataset, iscenario, n, k, num_epochs, model, irun, Te, E1, E2, E3, lr, lr_fac, reg_fac, t0, Estart, dropoutrate, multFactor)
